...
const totalCost = (email, orderId) => {
  ...
  let order = shop.customers[email].orders.find(order => order.id === orderId)
  let totalCost = 0
  // for (item of order.items){
  //   totalCost += productCost(item.productId) * item.qty
  // }
  order.items.map(item => totalCost += productCost(item.productId) * item.qty)
  return totalCost
}
